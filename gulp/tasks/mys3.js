...
module.exports = function (aws, options) {
    ...
    console.log("WWWWWWWWWW => aws client=" + JSON.stringify(client, null, 2));
    var waitTime = 0;
    ...
    return es.mapSync(function (file) {
        ...
        client.putBuffer(file.contents, uploadPath, headers, function (err, res) {
            if (err || res.statusCode !== 200) {
                console.log("WWWWWWWWWWWWWWWWWWTF " + JSON.stringify(err));
                gutil.log(gutil.colors.red('[FAILED]', file.path + " -> " + uploadPath));
            } else {
                console.log("WWWWWWWWWWWWWWWWWW SUCCESS " + JSON.stringify(res));
